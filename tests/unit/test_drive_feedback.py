...
def test_p09_two_sample_live_deficit_latches_bounded_tail_bias() -> None:
    ...
    active = feedback.update(
        ...
        actual_full12=_actual(spec.channel_index, second.reference_actual_deg),
        spec=spec,
    )
    restored = feedback.update(
        state_id="P09",
        motion_tick_index=spec.last_bias_tick + 1,
        actual_full12=_actual(spec.channel_index, second.reference_actual_deg),
        spec=spec,
    )
...
def test_p09_tail_bias_requires_both_live_deficit_samples() -> None:
    ...
    feedback.update(
        state_id="P09",
        motion_tick_index=first.motion_tick,
        actual_full12=_actual(spec.channel_index, first.reference_actual_deg),
        spec=spec,
    )
...
def test_p09_tail_bias_is_one_shot_across_a_same_state_retry() -> None:
    ...
    assert feedback.update(
        state_id="P09",
        motion_tick_index=spec.first_bias_tick,
        actual_full12=_actual(spec.channel_index, 0.0),
        spec=spec,
    ).active
