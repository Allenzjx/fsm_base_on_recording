...
def _reset_metadata(*, reused: bool) -> dict[str, object]:
    payload: dict[str, object] = {
        ...
        "reset_root_pose_writes": 1 if reused else 0,
        "reset_root_velocity_writes": 1 if reused else 0,
        "reset_joint_state_writes": 1 if reused else 0,
        "reset_global_simulation_resets": 0,
        "reset_simulation_forward_syncs": 1 if reused else 0,
        "settle_ticks": 180,
...
def test_reset_metadata_proves_step_free_reuse_without_global_reset() -> None:
    ...
    assert comparison["checks"]["reused_used_exactly_one_forward_sync"] is True
